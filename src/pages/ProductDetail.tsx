...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const wishlistItems = localStorage.getItem('wishlist') 
      ...
      : [];
    setIsInWishlist(wishlistItems.some((item: string) => item === id));
  }, [id]);
  ...
  const toggleWishlist = () => {
    ...
    if (isInWishlist) {
      const updatedWishlist = wishlistItems.filter((item: string) => item !== id);
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6 bg-white rounded-xl p-6 shadow-sm">
        ...
        <div className="lg:w-1/3 space-y-6 order-3">
          ...
          <button 
            className={`flex items-center justify-center gap-2 text-gray-600 hover:text-red-500 text-sm w-full py-3 border border-gray-200 rounded-lg transition-all ${
              isInWishlist ? 'bg-pink-50 text-red-500 border-pink-200' : ''
            }`}
      ...
      <div className="mt-12">
        <SuggestedProducts currentProductId={id || ''} limit={3} />
      </div>
      
      <RandomProducts currentProductId={id || ''} limit={3} />
