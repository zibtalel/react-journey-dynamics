...
BEGIN TRY
	...
	BEGIN
		...
		Into #ShortContact
		from crm.Contact where IsActive = 1 AND ContactType= 'Company'
		...
		JOIN [S].[External_ContactBusinessRelation] contactBR ON communication.[NavisionContactNo] COLLATE DATABASE_DEFAULT = contactBR.[Contact No_] COLLATE DATABASE_DEFAULT
		JOIN #ShortContact contact ON contactBR.[No_]  COLLATE DATABASE_DEFAULT = contact.LegacyId
		JOIN CRM.[Address] [address] ON contact.ContactId = [address].CompanyKey AND [address].IsActive = 1 AND [address].[AddressTypeKey]='1'
