...
BEGIN
	...
	BEGIN TRY
		...
		SET @tableName = 'SMS.ErrorCode';
			
		IF OBJECT_ID('tempdb..#ErrorCode') IS NOT NULL DROP TABLE #ErrorCode

		SELECT *				
		INTO #ErrorCode
		FROM [V].[External_ErrorCode]
		...
		BEGIN
			...
			MERGE [SMS].[ErrorCode] AS [target]
			USING #ErrorCode AS [source]
