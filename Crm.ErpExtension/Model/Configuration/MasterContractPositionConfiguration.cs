namespace Crm.ErpExtension.Model.Configuration
{
	...
	public class MasterContractPositionConfiguration : EntityConfiguration<MasterContractPosition>
	{
		public override void Initialize()
		{
			NestedProperty(x => x.Parent.DueDate, m =>
			{
				m.Filterable(f =>
				{
					f.Definition(new DateFilterDefinition { AllowPastDates = true, AllowFutureDates = false });
					f.Caption("T_FulFillmentDate");
				});
				m.Sortable(s => s.SortCaption("T_FulFillmentDate"));
			});
			...
			NestedProperty(x => x.Parent.OrderConfirmationDate, c =>
			{
		...
		public MasterContractPositionConfiguration(IEntityConfigurationHolder<MasterContractPosition> entityConfigurationHolder)
