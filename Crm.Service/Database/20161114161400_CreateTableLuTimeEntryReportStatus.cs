...
namespace Crm.Service.Database
{
	...
	public class CreateTableLuTimeEntryReportStatus : Migration
	{
		public override void Up()
		{
			if (!Database.TableExists("[LU].[TimeEntryReportStatus]"))
			{
				Database.ExecuteNonQuery(@"CREATE TABLE [LU].[TimeEntryReportStatus](
					[TimeEntryReportStatusId][int] IDENTITY(1, 1) NOT NULL,
				 ...
				 CONSTRAINT[PK_TimeEntryReportStatus] PRIMARY KEY CLUSTERED
				(
					[TimeEntryReportStatusId] ASC
				)WITH(PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON[PRIMARY]
				) ON[PRIMARY]");

				if (Database.TableExists("[LU].[TimeEntryReportStatus]") && Convert.ToInt32(Database.ExecuteScalar("SELECT COUNT(*) FROM [LU].[TimeEntryReportStatus]")) == 0)
				{
					Database.ExecuteNonQuery(@"INSERT INTO [LU].[TimeEntryReportStatus]
                        ([Name]
                        ,[Language]
                        ,[Value]
                        ,[Favorite]
